...
#
#
# # Local neighbourhood trajectories
# function smc_trajectory(em::ErgodicManagerR2, vtm::VTM, agent::Int; verbose::Bool=true, umax::Float64=1.0)
#
#     ud = VVF(tm.N)
#     xd = VVF(tm.N+1)
#     xd[1] = deepcopy(vtm[agent].x0)
#
#     # IDEA:  ADD COORDINATES AS WE GO INTO xy IN TM
#
#     #ck = zeros(em.K+1, em.K+1)
#     for n = 1:vtm[agent].N
#         # compute B
#         Bx, By = compute_B_L(em, xd, n, vtm[agent].h, vtm[agent].ck)
#         # normalize
#         den = sqrt(Bx*Bx + By*By)
#         ux = -umax * Bx / den
#         uy = -umax * By / den
#
#         ud[n] = [ux, uy]
#         xd[n+1] = integrate(vtm[agent], xd[n], ud[n])
#
#         vtm[agent] = xd[n+1]
#     end
#
...
#
# # TODO: this can be done a lot quicker
# function compute_B_L(em::ErgodicManagerR2, xd::VVF, n::Int, h::Float64, ck::Matrix{Float64})
#     Lx = em.domain.lengths[1]
#     Ly = em.domain.lengths[2]
#     dxn = xd[n][1] - x_min(em)
#     dyn = xd[n][2] - y_min(em)
